class op_2_targetSupplyAmmo: TaskDefaults {
	scope = 1;
	typeID = 2;
	...
	class TaskDetails {
		...
		description[] = {
			...
			,"<t size='1.1' color='#FFC600'>Brief:</t> <t>Recon indicates %8 have set up a weapons resupply point near %6. This will increase their operational capability to strike at key strategic locations in that area and should be eliminated before %8 support can be deployed.</t>"
			,"<t size='1.1' color='#FFC600'>Action:</t> <t>%7 forces will move to the area and conduct a sweep to locate and destroy the weapons, supply vehicles and any %8 opposition that occupy the area.</t>"
			,"<t size='1.1' color='#FFC600'>Intel:</t> <t>%8 forces are believed to have mobilised a large squad to operate in that area with small to medium arms while further heavier support is yet to arrive.</t>"
	...
	class Compositions {
		class AmmoSupply {
			position = "positionOffset";
			typeIDs[] = {"en_supply_ammo_1","en_supply_ammo_2"};
			targets[] = {"O_CargoNet_01_ammo_F"};
		};
	};
	class Groups {
		class EN_Vehicle_Target_1 {
			isTarget = 1;
			probability = 0.85;
			position = "positionOffset";
			distance[] = {50,100};
		...
		class EN_Vehicle_Target_2: EN_Vehicle_Target_1 {
			probability = 0.7;
			minPlayers = 8;
		};
	...
	class Objective {
		class Succeeded {
			...
			condition = "_targetsDestroyed && _targetsKilled";
		};
	};
};