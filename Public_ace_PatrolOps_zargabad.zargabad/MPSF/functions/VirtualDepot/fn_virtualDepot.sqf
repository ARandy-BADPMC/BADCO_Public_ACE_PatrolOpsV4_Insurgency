		...
		if (count _hardPoints == 0) then {
...

			{
				if ((_forEachIndex + 1) % 2 == 0) then {
					_hardPoints set [_forEachIndex,[(_leftIndex * _step) - (_sizeX/(count _pylons)),0,-1.5]];
					_leftIndex = _leftIndex + 1;
				} else {
					_hardPoints set [_forEachIndex,[(_rightIndex * _step) - (_sizeX/(count _pylons)),0,-1.5]];
...
// Functions
	...
	case "deployCenter" : {
		...
		_center call BADCO_texture_applier; //setting heli/car skin A.Randy
